...
func NewHeaderProtectorFromIntroKey(introKey []byte, headerType HeaderType) (*HeaderProtector, error) {
	log.WithField("headerType", headerType).Debug("NewHeaderProtectorFromIntroKey: creating protector with intro key for both keys")
...
func (hp *HeaderProtector) validatePacketSize(packet []byte) error {
	log.WithField("packetLen", len(packet)).Debug("validatePacketSize: checking minimum packet size")
...
func (hp *HeaderProtector) applyXORMasks(packet []byte) error {
	log.WithField("packetLen", len(packet)).Debug("applyXORMasks: applying XOR masks to header bytes 0-15")
...
func (hp *HeaderProtector) generateMask(key, nonce []byte) ([]byte, error) {
	log.Debug("generateMask: generating 8-byte XOR mask via ChaCha20")
...
func (hp *HeaderProtector) encryptLongHeaderExtension(packet []byte) error {
	log.WithField("headerType", hp.headerType).Debug("encryptLongHeaderExtension: encrypting long header extension with ChaCha20")
...
func NewHeaderProtectorManager(introKey, remoteIntroKey []byte, isInitiator bool) (*HeaderProtectorManager, error) {
	log.WithField("isInitiator", isInitiator).Debug("NewHeaderProtectorManager: creating header protector manager")
...
func (hpm *HeaderProtectorManager) GetProtectorForType(headerType HeaderType) (*HeaderProtector, error) {
	log.WithField("headerType", headerType).Debug("GetProtectorForType: selecting header protector for packet type")
	hpm.mu.RLock()
...
func (hpm *HeaderProtectorManager) keysForIntroProtected() (k1, k2 []byte, err error) {
	log.WithField("isInitiator", hpm.isInitiator).Debug("keysForIntroProtected: resolving intro-protected header keys")
	if hpm.isInitiator {
