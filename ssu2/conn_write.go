...
func (h *SSU2Conn) Write(b []byte) (int, error) {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "Write", "dataLen": len(b)}).Debug("sending data")
...
func (h *SSU2Conn) buildI2NPFragmentBlocks(data []byte, maxBlockData int) ([]*SSU2Block, error) {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "buildI2NPFragmentBlocks", "dataLen": len(data), "maxBlockData": maxBlockData}).Debug("fragmenting I2NP message")
	const (
...
func (h *SSU2Conn) WriteBlocks(blocks []*SSU2Block) error {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "WriteBlocks", "blockCount": len(blocks)}).Debug("sending blocks")
	if err := h.validateReadyForIO(); err != nil {
		return err
	}
...
func (h *SSU2Conn) newDataPacket() *SSU2Packet {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "newDataPacket"}).Debug("allocating data packet")
...
func (h *SSU2Conn) writeBlock(block *SSU2Block) error {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "writeBlock", "blockType": block.Type}).Debug("sending block")
...
func (h *SSU2Conn) sendLoop() {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "sendLoop"}).Debug("starting outbound packet transmission loop")
	defer h.wg.Done()
...
func (h *SSU2Conn) retransmitLoop() {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "retransmitLoop"}).Debug("starting retransmit loop")
	defer h.wg.Done()
...
func (h *SSU2Conn) retransmitExpired() {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "retransmitExpired"}).Debug("checking pending packets for retransmission")
	now := time.Now()
...
func (h *SSU2Conn) sendPacketDirect(packet *SSU2Packet) error {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "sendPacketDirect", "pktNum": packet.PacketNumber, "msgType": packet.MessageType}).Debug("sending packet")
...
func (h *SSU2Conn) encryptDataPayload(packet *SSU2Packet) ([]byte, error) {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "encryptDataPayload", "pktNum": packet.PacketNumber, "payloadLen": len(packet.Payload)}).Debug("encrypting payload")
...
func (h *SSU2Conn) applySipHashObfuscation(packet *SSU2Packet) {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "applySipHashObfuscation", "msgType": packet.MessageType}).Debug("applying length obfuscation")
...
func (h *SSU2Conn) applyOutboundHeaderProtection(data []byte, messageType uint8) error {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "applyOutboundHeaderProtection", "msgType": messageType}).Debug("encrypting header")
...
func (h *SSU2Conn) trackPendingPacket(packet *SSU2Packet, plaintext []byte) {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "trackPendingPacket", "pktNum": packet.PacketNumber}).Debug("recording packet for retransmission")
...
func (h *SSU2Conn) sendImmediateACK() {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "sendImmediateACK"}).Debug("generating and sending immediate ACK")
...
func (h *SSU2Conn) handleCongestionBlock(flags uint8) error {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "handleCongestionBlock", "flags": flags}).Debug("processing congestion block")
...
func (h *SSU2Conn) handlePathChallengeData(data []byte) error {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "handlePathChallengeData", "dataLen": len(data)}).Debug("processing path challenge")
...
func (h *SSU2Conn) handlePathResponseData(data []byte) error {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "handlePathResponseData", "dataLen": len(data)}).Debug("processing path response")
...
func (h *SSU2Conn) nextSendSequence() uint32 {
	...
	if h.sendSequence == 0xFFFFFFFF {
		log.WithFields(logger.Fields{"pkg": "ssu2", "func": "nextSendSequence"}).Error("packet number exhausted (0xFFFFFFFF): closing connection per SSU2 spec")
		go h.Close()
		return 0xFFFFFFFF
	}
...
func (h *SSU2Conn) initiateRekey() {
	...
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "initiateRekey"}).Warn("initiating NextNonce rekey (unfinalized spec area — interoperability not guaranteed)")
	...
	if err != nil {
		log.WithFields(logger.Fields{"pkg": "ssu2", "func": "initiateRekey", "error": err}).Error("failed to derive rekey for send cipher")
		return
	}

	// Send NextNonce block encrypted with the OLD key before rekeying.
	if err := h.sendNextNonceInline(); err != nil {
		log.WithFields(logger.Fields{"pkg": "ssu2", "func": "initiateRekey", "error": err}).Error("failed to send NextNonce block")
		return
	}
...
func (h *SSU2Conn) sendNextNonceInline() error {
	log.WithFields(logger.Fields{"pkg": "ssu2", "func": "sendNextNonceInline"}).Debug("building and sending NextNonce block")
